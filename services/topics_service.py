...
def create_topic(topic: TopicCreate, user_id: int):
    new_id = insert_query('''INSERT INTO topics (title, user_id, category_id) VALUES (?, ?, ?)''',
        (topic.title, user_id, topic.category_id))

    data = read_query('''SELECT id, title, user_id, category_id, best_reply_id 
